...
PROJECT_NAME=trems
STACK_NAME=trems-patyamasaki-tangrama-com-br
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend --build-arg PLONE_VERSION=${PLONE_VERSION} -t patyamasaki/trems-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend --build-arg VOLTO_VERSION=${VOLTO_VERSION} -t patyamasaki/trems-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name trems-backend-acceptance -d patyamasaki/trems-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name trems-frontend-acceptance --link trems-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d patyamasaki/trems-frontend:acceptance
